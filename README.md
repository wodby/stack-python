# Python application stack for Kubernetes on Wodby

Deploy Python applications on Kubernetes with Wodby.

This repository defines the Wodby stack manifests and default service
composition for Python.
...
- [Python service](https://github.com/wodby/service-python)
...
| --- | --- |
| Python<br>`python` | required; enabled by default; main service; links: `db` → `postgres`, `redis` → `valkey`, `sendmail` → `mailpit` |
...
Start from [Python boilerplate](https://github.com/wodby/python-boilerplate), or connect your own compatible source
repository.
